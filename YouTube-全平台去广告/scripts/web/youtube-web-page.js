 ...
 * Scope: Web only. 用于 Safari、Chrome 及移动网页，不处理原生 App protobuf。
 * Client boundary: 当前由 Surge 模块加载；Stash、Shadowrocket 适配需单独验证。
 * Shared behavior: 广告节点清理、播放器广告态识别、静音加速、结束后状态恢复。
 */
...
if (!source || !/<\/body>/i.test(source)) {
  ...
  const injectedScript = `<script${nonceAttribute} data-youtube-adblock-skipper>(()=>{
    ...
    new MutationObserver(scheduleCleanPage).observe(document.documentElement,{subtree:true,childList:true});
    document.addEventListener?.("yt-navigate-finish",scheduleCleanPage,true);
    document.addEventListener?.("yt-page-data-updated",scheduleCleanPage,true);
    document.addEventListener?.("visibilitychange",()=>{
      if(isPageVisible())safeCleanPage();
    },true);
    window.addEventListener?.("popstate",scheduleCleanPage,true);
    setInterval(()=>{
