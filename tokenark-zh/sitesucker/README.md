...
版本门禁：Bundle ID `us.sitesucker.mac.sitesucker`，版本 `6.1.8`。
...
如果系统应用由 `root:wheel` 所有，脚本会自动使用 `~/Applications/SiteSucker.app` 用户副本并进行 ad-hoc 签名。下载 URL、文件名、User-Agent 和站点内容按边界保留原文。
